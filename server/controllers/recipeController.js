...
class RecipeController {
    ...
    async update(req, res){
            ...
            const {id} = req.params
            const {name, time, complex, profile_mini, profile, typeId, userId, kcal, protein, fat, carb} = req.body
            const {img} = req.files
            const fileName = uuid.v4() + ".jpg"
            img.mv(path.resolve(__dirname, '..', 'static', fileName))
            const recipe = await ( await (Recipe.findOne({where: {id}}))).update({name:name, time:time, complex:complex, profile_mini:profile_mini, profile:profile, typeId:typeId, userId:userId, kcal:kcal, protein:protein, fat:fat, carb:carb})
            return res.json({recipe})
